...
def is_descending(seq: list[int]) -> bool:
    return all(seq[i] > seq[i + 1] for i in range(len(seq) - 1))
...
def fission_report_direction(report: list[int]) -> ReportDirection | None:
    ...
    for report in reports:
        if is_fission_report_safe(report):
            ...
            for idx in range(len(report)):
                if is_fission_report_safe(
                    [x for i, x in enumerate(report) if i != idx]
                ):
                    safe_reports = safe_reports + 1
                    break
...
fission_reports = get_fission_reports("input2.txt")
