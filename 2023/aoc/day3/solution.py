...
def part_one():
    class Schematic:
        ...
        def has_symbol_around(
            ...
            if not is_beginning:
                if self._rows[row_number][col_num_start - 1] != "." and not self.is_int(
                    self._rows[row_number][col_num_start - 1]
                ):
                    return True
            if not is_end:
                if self._rows[row_number][col_num_end + 1] != "." and not self.is_int(
                    self._rows[row_number][col_num_end + 1]
                ):
                    return True
            if not is_top:
                j = col_num_start
                ...
                while j <= col_num_end + 1:
                    ...
                    if j > col_num_end and is_end:
                        j += 1
                        continue
                    if self._rows[row_number - 1][j] != "." and not self.is_int(
                        self._rows[row_number - 1][j]
                    ):
                        return True
                    j += 1
            if not is_bottom:
                ...
                while k <= col_num_end + 1:
                    if k > col_num_end and is_end:
                        k += 1
                        continue
                    if self._rows[row_number + 1][k] != "." and not self.is_int(
                        self._rows[row_number + 1][k]
                    ):
                        return True
                    k += 1
            return False
...
def part_two():
    ...
    class Schematic:
        ...
        def num_find_when_directly_above_or_below(
