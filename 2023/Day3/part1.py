...
class Schematic:
    ...
    def analyze_row(self, row_number: int):
        ...
        while col_number < len(self._rows[row_number]):
            start = col_number
            if (not self.is_int(self._rows[row_number][col_number])):
                col_number += 1
                continue
            col_number += 1
            while (self.is_int(self._rows[row_number][col_number])):
                col_number += 1
            if start == 0:
                is_beginning = True

            if row_number == 7 and start == 6 and col_number-1 == 8:
    ...
    def has_symbol_around(self, row_number: int, col_num_start: int, col_num_end: int, is_top: bool, is_bottom: bool, is_beginning: bool, is_end: bool) -> bool:
        if row_number == 7 and col_num_start == 6 and col_num_end == 8:
            #import pdb; pdb.set_trace()
            print()
        if not is_beginning:
